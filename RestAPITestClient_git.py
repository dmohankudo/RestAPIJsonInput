...
class API(object):
    ...
    def post_request(self,
        ...
        try:
            self.response = self.session.post(url=self.endpoint_url,
                                              json=_data_json)
    ...
    def invoke_api(self,
        ...
        info('post data is:{data}'.format(data=_data_json))
        self.post_request(_data_json=self.tc_post_data)
        info('response is: {r}-{code}'.format(r=self.response.json(),
    ...
    def verify_response(self,
        ...
        if _status_code:
            if 'statusCode' in resp_json:
                (bool_stat1,
                 exp1,
                 act1) = compare_values(_value1=_status_code,
                                        _value2=resp_json['statusCode'])
                info('Both equal?->{status}-Expected->{exp}-Actual->{act}'.format(status=bool_stat1,
                                                   exp=exp1,
                                                   act=act1))
        if _status_message:
            if 'statusMessage' in resp_json:
                ...
                info('Both equal?->{status}-Expected->{exp}-Actual->{act}'.format(status=bool_stat2,
                                                   exp=exp2,
                                                   act=act2))
        if False in (bool_stat1, bool_stat2):
            self.tc_status = 'Fail'
            error('TC Desc:{d}TC status:{s}'.format(s=self.tc_status,
                                                    d=_tc_desc))
        else:
            self.tc_status = 'Pass'
            info('TC Desc:{d}TC status:{s}'.format(s=self.tc_status,
                                                   d=_tc_desc))

    def testAPI(self,
                _data_json=None,
                _exp_statusCode=None,
                _exp_statusMessage=None):
        info('-' * 40)
        self.tc_data = _data_json
        self.tc_desc = self.tc_data['desc']
        self.tc_post_data = self.tc_data['post_data']
        self.tc_exp_result = self.tc_data['expected']
        info('TestCae Desc: {desc}'.format(desc=self.tc_desc))
        self.invoke_api(_data_json=self.tc_post_data)
        if self.response.status_code in (200, 400, 415):
            self.verify_response(_status_code=self.tc_exp_result['status_code'],
                                 _status_message=self.tc_exp_result['status_message'],
